  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#0B1121]/90 backdrop-blur-md border border-slate-700/50 p-4 rounded-xl shadow-2xl z-50">
        <p className="text-xs text-slate-400 mb-3 font-medium uppercase tracking-wider">Relevé : {formatTime(label)}</p>
        {payload.map((entry: any, index: number) => (
          <p key={index} className="font-bold text-sm flex items-center gap-3 mb-1" style={{ color: entry.stroke }}>
            <span className="w-2.5 h-2.5 rounded-full shadow-sm" style={{ backgroundColor: entry.stroke, boxShadow: `0 0 8px ${entry.stroke}` }}></span>
              ...
              <div className="space-y-6">
                <div className="bg-[#0F172A]/60 backdrop-blur-sm border border-slate-800 p-8 rounded-3xl shadow-xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Nom du Client</label><input type="text" name="nomClient" value={clientConfig.nomClient} onChange={handleClientChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all outline-none" /></div>
                    <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Responsable</label><input type="text" name="responsable" value={clientConfig.responsable} onChange={handleClientChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all outline-none" /></div>
                    <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Localisation</label><input type="text" name="localisation" value={clientConfig.localisation} onChange={handleClientChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all outline-none" /></div>
                    <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Réf. Projet</label><input type="text" name="referenceProjet" value={clientConfig.referenceProjet} onChange={handleClientChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all outline-none" /></div>
                  </div>
                </div>
                ...
                <div className="bg-[#0F172A]/60 backdrop-blur-sm border border-slate-800 p-8 rounded-3xl shadow-xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="space-y-5">
                      <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">ID Tronçon</label><input type="text" name="idTroncon" value={pipeConfig.idTroncon} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-blue-500 outline-none" /></div>
                      <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Secteur</label><input type="text" name="zoneAEP" value={pipeConfig.zoneAEP} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-blue-500 outline-none" /></div>
                    </div>
                    <div className="space-y-5">
                      <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Matériau</label><select name="materiau" value={pipeConfig.materiau} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-blue-500 outline-none"><option value="FONTE">Fonte Ductile</option><option value="PEHD">PEHD</option><option value="PVC">PVC</option><option value="ACIER">Acier</option></select></div>
                      <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Rugosité (mm)</label><input type="number" step="0.001" name="rugosite" value={pipeConfig.rugosite} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-cyan-300 p-3 text-sm rounded-xl focus:border-blue-500 outline-none font-mono" /></div>
                    </div>
                    <div className="space-y-5">
                      <div className="grid grid-cols-2 gap-3">
                        <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">DN</label><input type="number" name="diametreNominal" value={pipeConfig.diametreNominal} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-blue-500 outline-none" /></div>
                        <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">PN</label><input type="number" name="pressionPN" value={pipeConfig.pressionPN} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-blue-500 outline-none" /></div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Z Amont</label><input type="number" name="altitudeAmont" value={pipeConfig.altitudeAmont} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-blue-500 outline-none" placeholder="m" /></div>
                        <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Z Aval</label><input type="number" name="altitudeAval" value={pipeConfig.altitudeAval} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-slate-800/80 text-white p-3 text-sm rounded-xl focus:border-blue-500 outline-none" placeholder="m" /></div>
                      </div>
                    </div>
                    <div className="space-y-5 flex flex-col justify-end">
                      <div><label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block mb-2">Débit Cible (L/min)</label><input type="number" name="debitConsigne" value={pipeConfig.debitConsigne} onChange={handlePipeChange} className="w-full bg-[#060B14] border border-blue-500/50 text-blue-400 p-3 text-sm rounded-xl focus:border-blue-500 outline-none font-bold text-lg" /></div>
